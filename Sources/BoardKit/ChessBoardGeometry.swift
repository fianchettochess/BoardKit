...
/// h8=63) while `Position.board` is rank-major — these helpers own that
...
public enum ChessBoardGeometry {
    ...
    /// physical board's frame can be translated into the consumer's frame.
    ...
    /// Square names (a1..h8) whose occupancy disagrees between the expected
    /// `position` and the board's reported file-major `boardOccupancy`.
    ...
    public static func mismatchedSquares(position: Position, boardOccupancy: [Bool]) -> [String] {
        ...
        for file in 0..<8 {
            for rank in 0..<8 {
                // Position stores [Piece?] in rank-major order: index = rank * 8 + file.
